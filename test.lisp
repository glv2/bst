...
(test bst-search
  ...
  (let* ((*bst-copy-function* #'copy-seq)
    ...
    (multiple-value-bind (value found) (bst-search tree '(5 5 5))
      (is (bst-equal-p '(5 5 5) value))
      (is-true found))
    (multiple-value-bind (value found) (bst-search tree nil)
      (is (bst-equal-p nil value))
      (is-true found))
    (multiple-value-bind (value found) (bst-search tree '(1))
      (is-false value)
      (is-false found))))
...
(test bst-max-value
  ...
  (let* ((*bst-copy-function* #'copy-seq)
         ...
         (*bst-lesser-p-function* (lambda (x y) (< (length x) (length y))))
         (tree (bst-from-values '((1 2 3 4) (5 5 5) (9 8 7 6 5 4) ()))))
    (multiple-value-bind (value found) (bst-max-value tree)
      (is (bst-equal-p '(9 8 7 6 5 4) value))
      (is-true found))))
...
(test bst-min-value
  ...
  (let* ((*bst-copy-function* #'copy-seq)
         ...
         (*bst-lesser-p-function* (lambda (x y) (< (length x) (length y))))
         (tree (bst-from-values '((1 2 3 4) (5 5 5) (9 8 7 6 5 4) ()))))
    (multiple-value-bind (value found) (bst-min-value tree)
      (is (bst-equal-p nil value))
      (is-true found))))
...
(test bst-map
  (flet ((collect (tree)
           (let ((values '()))
             ...
             (nreverse values))))
    (is (equalp '() (collect +bst-empty+)))
    (is (equalp '(1) (collect (bst-from-values '(1)))))
    ...
    (let* ((*bst-copy-function* #'copy-seq)
           ...
           (*bst-lesser-p-function* (lambda (x y) (< (length x) (length y))))
           (tree (bst-from-values '((1 2 3 4) (5 5 5) (9 8 7 6 5 4) ()))))
      (is (equalp '(() (5 5 5) (1 2 3 4) (9 8 7 6 5 4)) (collect tree))))))
